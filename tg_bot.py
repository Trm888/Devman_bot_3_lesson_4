...
def new_question(update: Update, context: CallbackContext, quiz_info, redis_db):
...
def give_up(update: Update, context: CallbackContext, redis_db):
...
def main():
    ...
    partial_start = partial(start, redis_db=redis_db)
    partial_new_question = partial(new_question, redis_db=redis_db, quiz_info=questions_answers)
    partial_give_up = partial(give_up, redis_db=redis_db)
    ...
    conv_handler = ConversationHandler(
        entry_points=[CallbackQueryHandler(partial_new_question, pattern='new_question')],
        states={
            START: [
                MessageHandler(Filters.text & (~Filters.command), partial_answer),
            ],
        },
        fallbacks=[CommandHandler("start", start), CallbackQueryHandler(partial_give_up, pattern='give_up')],
    )

    dp.add_handler(conv_handler)
    dp.add_handler(CommandHandler("start", partial_start))
